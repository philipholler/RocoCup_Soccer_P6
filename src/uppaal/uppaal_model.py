...
class UppaalStrategy:
    # This class represent a UPPAAL strategy which is
    # generated by modifying the model file
    # executing the UPPAAL verifier
    # and then parsing the output file
    ...
    def __init__(self, strategy_name: str) -> None:
        ...
        # The UPPAAL output files contains 'regressors' that link a certain state to an action
        # Each link has an associated weight that indicates the predicted reward for choosing that action
        # The following dictionaries keep track of regressor indexes and the action that corresponds to the regressor
    ...
    def _extract_location_ids(self, strategy: str):
        # Maps the index of location to the name of the location
        location_name_to_id = {}
    ...
    def add_location_mappings(location_to_index, template_name, mappings):
        # Maps the states(locations) in the model to their respective id(index)
    ...
    def _extract_transition_dict(strat_string):
        # Maps the index of transitions to their id
...
class Regressor:
    # Represents a single regressor from the UPPAAL output file
    # A regressor is identified by a combination of states
    # A regressor contains a set of transitions with an associated value indicating the estimated reward of the
    ...
    def get_lowest_val_trans(self):
        # Extracts the transition with the lowest estimated reward
        lowest = None
    ...
    def get_highest_val_trans(self):
        # Extracts the transition with the highest estimated reward
        highest = None
        for pair in self.trans_val_pairs:
            if highest is None:
                highest = pair
...
class GlobalDeclaration:
    # The UPPAAL model files contains global variable declarations that need to be altered in order to
    # represent the current state of the game

    # A declaration has a type, an identifier and a value
